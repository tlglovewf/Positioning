#ifndef _ORBINITIALIZER_H_
#define _ORBINITIALIZER_H_


#include "P_ORBFrame.h"


namespace Position
{

// THIS IS THE INITIALIZER FOR MONOCULAR SLAM. NOT USED IN THE STEREO OR RGBD CASE.
class Initializer
{
    typedef pair<int,int> Match;

public:

    // Fix the reference frame
    Initializer(const ORBFrame &ReferenceFrame, float sigma = 2.0, int iterations = 200);

    // Computes in parallel a fundamental matrix and a homography
    // Selects a model and tries to recover the motion and the structure from motion
    bool Initialize(const ORBFrame &CurrentFrame, const vector<int> &vMatches12,
                    cv::Mat &R21, cv::Mat &t21, vector<cv::Point3f> &vP3D, BolVector &vbTriangulated);


private:

    void FindHomography(BolVector &vbMatchesInliers, float &score, cv::Mat &H21);
    void FindFundamental(BolVector &vbInliers, float &score, cv::Mat &F21);

    cv::Mat ComputeH21(const vector<cv::Point2f> &vP1, const vector<cv::Point2f> &vP2);
    cv::Mat ComputeF21(const vector<cv::Point2f> &vP1, const vector<cv::Point2f> &vP2);

    float CheckHomography(const cv::Mat &H21, const cv::Mat &H12, BolVector &vbMatchesInliers, float sigma);

    float CheckFundamental(const cv::Mat &F21, BolVector &vbMatchesInliers, float sigma);

    bool ReconstructF(BolVector &vbMatchesInliers, cv::Mat &F21, cv::Mat &K,
                      cv::Mat &R21, cv::Mat &t21, vector<cv::Point3f> &vP3D, BolVector &vbTriangulated, float minParallax, int minTriangulated);

    bool ReconstructH(BolVector &vbMatchesInliers, cv::Mat &H21, cv::Mat &K,
                      cv::Mat &R21, cv::Mat &t21, vector<cv::Point3f> &vP3D, BolVector &vbTriangulated, float minParallax, int minTriangulated);

    void Triangulate(const cv::KeyPoint &kp1, const cv::KeyPoint &kp2, const cv::Mat &P1, const cv::Mat &P2, cv::Mat &x3D);

    void Normalize(const KeyPtVector &vKeys, vector<cv::Point2f> &vNormalizedPoints, cv::Mat &T);

    int CheckRT(const cv::Mat &R, const cv::Mat &t, const KeyPtVector &vKeys1, const KeyPtVector &vKeys2,
                       const vector<Match> &vMatches12, BolVector &vbInliers,
                       const cv::Mat &K, vector<cv::Point3f> &vP3D, float th2, BolVector &vbGood, float &parallax);

    void DecomposeE(const cv::Mat &E, cv::Mat &R1, cv::Mat &R2, cv::Mat &t);


    // Keypoints from Reference Frame (Frame 1)
    KeyPtVector mvKeys1;

    // Keypoints from Current Frame (Frame 2)
    KeyPtVector mvKeys2;

    // Current Matches from Reference to Current
    vector<Match> mvMatches12;
    BolVector mvbMatched1;

    // Calibration
    cv::Mat mK;

    cv::Mat refImg;

    // Standard Deviation and Variance
    float mSigma, mSigma2;

    // Ransac max iterations
    int mMaxIterations;

    // Ransac sets
    vector< SzVector > mvSets;   

};

} 

#endif // _ORBINITIALIZER_H_
