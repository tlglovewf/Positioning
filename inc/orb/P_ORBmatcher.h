...
public:
    ...
    int SearchByProjection(ORBFrame &F, const MapPtVector &vpMapPoints, const float th=3);
    ...
    int SearchByProjection(ORBFrame &CurrentFrame, ORBKeyFrame* pKF, const MapPtSet &sAlreadyFound, const float th, const int ORBdist);

    // Project MapPoints using a Similarity Transformation and search matches.
    // Used in loop detection (Loop Closing)
     int SearchByProjection(ORBKeyFrame* pKF, cv::Mat Scw, const MapPtVector &vpPoints, MapPtVector &vpMatched, int th);

    // Search matches between MapPoints in a ORBKeyFrame and ORB in a ORBFrame.
    // Brute force constrained to ORB that belong to the same vocabulary node (at a certain level)
    // Used in Relocalisation and Loop Detection
    int SearchByBoW(ORBKeyFrame *pKF, ORBFrame &F, MapPtVector &vpMapPointMatches);
    ...
    int SearchBySim3(ORBKeyFrame* pKF1, ORBKeyFrame* pKF2, MapPtVector &vpMatches12, const float &s12, const cv::Mat &R12, const cv::Mat &t12, const float th);

    // Project MapPoints into ORBKeyFrame and search for duplicated MapPoints.
    int Fuse(ORBKeyFrame* pKF, const MapPtVector &vpMapPoints, const float th=3.0);

    // Project MapPoints into ORBKeyFrame using a given Sim3 and search for duplicated MapPoints.
    int Fuse(ORBKeyFrame* pKF, cv::Mat Scw, const MapPtVector &vpPoints, float th, MapPtVector &vpReplacePoint);
