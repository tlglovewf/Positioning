...
int Optimizer::PoseOptimization(ORBFrame *pFrame)
{
    g2o::SparseOptimizer optimizer;
    g2o::BlockSolver_6_3::LinearSolverType * linearSolver;
    ...
    {
    unique_lock<mutex> lock(ORBMapPoint::mGlobalMutex);
...
{    
    ...
    const vector<ORBKeyFrame*> vNeighKFs = pKF->GetVectorCovisibleKeyFrames();
    for(int i=0, iend=vNeighKFs.size(); i<iend; i++)
    {
        ORBKeyFrame* pKFi = vNeighKFs[i];
    ...
    // Set Local ORBKeyFrame vertices
    for(list<ORBKeyFrame*>::iterator lit=lLocalKeyFrames.begin(), lend=lLocalKeyFrames.end(); lit!=lend; lit++)
    {
    ...
    // Set Fixed ORBKeyFrame vertices
    for(list<ORBKeyFrame*>::iterator lit=lFixedCameras.begin(), lend=lFixedCameras.end(); lit!=lend; lit++)
    {
        ORBKeyFrame* pKFi = *lit;
        g2o::VertexSE3Expmap * vSE3 = new g2o::VertexSE3Expmap();
        vSE3->setEstimate(PConverter::toSE3Quat(pKFi->getPose()));
    ...
    vector<ORBKeyFrame*> vpEdgeKFMono;
    vpEdgeKFMono.reserve(nExpectedSize);

    vector<ORBMapPoint*> vpMapPointEdgeMono;
    vpMapPointEdgeMono.reserve(nExpectedSize);

    vector<g2o::EdgeStereoSE3ProjectXYZ*> vpEdgesStereo;
    vpEdgesStereo.reserve(nExpectedSize);

    vector<ORBKeyFrame*> vpEdgeKFStereo;
    ...
    for(size_t i=0, iend=vpEdgesMono.size(); i<iend;i++)
    {
        ...
        ORBMapPoint* pMP = vpMapPointEdgeMono[i];
    ...
    for(size_t i=0, iend=vpEdgesStereo.size(); i<iend;i++)
    {
        ...
        ORBMapPoint* pMP = vpMapPointEdgeStereo[i];
    ...
    vector<pair<ORBKeyFrame*,ORBMapPoint*> > vToErase;
    ...
    for(size_t i=0, iend=vpEdgesStereo.size(); i<iend;i++)
    {
        ...
        if(e->chi2()>7.815 || !e->isDepthPositive())
        {
            ORBKeyFrame* pKFi = vpEdgeKFStereo[i];
            vToErase.push_back(make_pair(pKFi,pMP));
        }
    }
                                       ...
                                       const map<ORBKeyFrame *, set<ORBKeyFrame *> > &LoopConnections, const bool &bFixScale)
{
    ...
    for(size_t i=0, iend=vpKFs.size(); i<iend; i++)
    {
        ...
        ORBKeyFrame* pParentKF = pKF->GetParent();
        ...
        // Loop edges
        const set<ORBKeyFrame*> sLoopEdges = pKF->GetLoopEdges();
        for(set<ORBKeyFrame*>::const_iterator sit=sLoopEdges.begin(), send=sLoopEdges.end(); sit!=send; sit++)
        {
            ORBKeyFrame* pLKF = *sit;
        ...
        const vector<ORBKeyFrame*> vpConnectedKFs = pKF->GetCovisiblesByWeight(minFeat);
        for(vector<ORBKeyFrame*>::const_iterator vit=vpConnectedKFs.begin(); vit!=vpConnectedKFs.end(); vit++)
        {
            ORBKeyFrame* pKFn = *vit;
...
int Optimizer::OptimizeSim3(ORBKeyFrame *pKF1, ORBKeyFrame *pKF2, MapPtVector &vpMatches1, g2o::Sim3 &g2oS12, const float th2, const bool bFixScale)
{
    ...
    for(size_t i=0; i<vpEdges12.size();i++)
    {
        ...
        if(e12->chi2()>th2 || e21->chi2()>th2)
        {
            ...
            vpMatches1[idx]=static_cast<ORBMapPoint*>(NULL);
