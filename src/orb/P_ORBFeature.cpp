...
void ORBextractor::ComputeKeyPointsOld(std::vector<KeyPtVector> &allKeypoints)
{
    allKeypoints.resize(nlevels);
    ...
    for (int level = 0; level < nlevels; ++level)
    {
        ...
        vector<vector<KeyPtVector> > cellKeyPoints(levelRows, vector<KeyPtVector>(levelCols));

        vector<vector<int> > nToRetain(levelRows,vector<int>(levelCols,0));
        vector<vector<int> > nTotal(levelRows,vector<int>(levelCols,0));
        vector<BolVector > bNoMore(levelRows,BolVector(levelCols,false));
        ...
        for(int i=0; i<levelRows; i++)
        {
            for(int j=0; j<levelCols; j++)
            {
                KeyPtVector &keysCell = cellKeyPoints[i][j];
...
static void computeDescriptors(const Mat& image, KeyPtVector& keypoints, Mat& descriptors,
...
void ORBextractor::operator()( InputArray _image, InputArray _mask, KeyPtVector& _keypoints,
...
{ 
    ...
    vector < KeyPtVector > allKeypoints;
