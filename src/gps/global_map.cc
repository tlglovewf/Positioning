...
namespace gm{
    ...
    void gm::GlobalMap::CalConnections()
    {
        ...
        {
            std::map<std::shared_ptr<gm::Frame>, int> frame_list;//<与当前帧有共视的frame,共视点的数量>
...
//                         }
            ...
            for(std::map<std::shared_ptr<gm::Frame>,int>::iterator mit=frame_list.begin(), mend=frame_list.end(); mit!=mend; mit++)//遍历frame_list，即遍历所有与当前帧共视的帧
            {
...
//                 }
        ...
        for(size_t i=0; i<mappoints.size(); i++)
        {
            if(mappoints[i]->id==id)
            {
                ...
                for(size_t j=0; j<mp_p->track.size(); j++)
                {
                    ...
                    assert(frame_p->obss.size() > mp_p->track[j].kp_ind);
    ...
    {
        std::set<std::shared_ptr<gm::Frame>> all_frame_set;//地图中所有的帧
        for(int i=0; i<frames.size(); i++)
        {
            all_frame_set.insert(frames[i]);
        }
    ...
    {
        submap=*(this);//将当前全局地图赋值给子图submap
        //清空子图的frame和mappoint
        submap.frames.clear();
        submap.mappoints.clear();
        std::map<long unsigned int, std::shared_ptr<gm::Frame>> old_to_new_frame_map;//<全局图中的frameid,frame>存储所有放入了子图的frame
        //将startframe_id 到 endframe_id的帧添加至子图submap
        ...
        //调整子图中frames的imu_next_frame的id到子图中的id
        for(size_t i=0; i<submap.frames.size(); i++)//遍历子图的所有frames
        {
            if(submap.frames[i]->imu_next_frame!=nullptr)//如果该帧有imu_next_frame
            {
        ...
        //将mappoint放入子图
        for(size_t i=0; i<mappoints.size(); i++)//遍历全局图中的所有mappoint
        {
            for(size_t j=0; j<mappoints[i]->track.size(); j++)//对于每一个mappoint，遍历能观测到该点的所有帧
            {
                if(old_to_new_frame_map.count(mappoints[i]->track[j].frame->id)!=0)//如果该帧在子图中存在
                {
