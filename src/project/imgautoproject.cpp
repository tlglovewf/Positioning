...
ImgAutoConfig::ImgAutoConfig(const std::string &path) : PrjPath(""),
                                                        CamMatrixPath("")
{
    PUSH_MAP(PrjPath);
...
//相机配置参数
Position::CameraParam CameraCfgDB::getCameraParams()
{
...
//加载track json
bool ImgAutoData::loadTrackJson(const std::string &path, Position::FrameDataPtrVector &framedatas)
{
                ...
                if (dataArray.IsArray())
                {
                    ...
                    if (dataValue.HasMember("trackMark"))
                    {
                        ...
                        if (trmkArray.IsArray())
                        {
                            ...
                            {
                                Position::PoseData &item = framedatas[j]->_pos;
                                int mseq;
...
//读取单个图片trk文档
void ImgAutoData::ParseTrkTxt(const string &txtfile, Position::StringVector &lines)
{
...
bool ImgAutoData::loadDatas()
{
    ...
    try
    {
        ...
        for (size_t i = 0; i < mFrameDatas.size(); ++i)
        {
            ...
            Position::PUtils::ReplaceFileSuffix(trkfile, "jpg", "trk");
            trkfile = trackrestpath + trkfile;
            Position::StringVector lines;
            ...
            { //数据不为空,则表示当前帧没有识别的目标
                ...
                { //遍历目标
                    //根据拆分字符 拆分字符串
                    Position::StringVector values = Position::PUtils::SplitString(item, SPLITSTR);
...
static inline Position::InfoIndex parseIndex(const std::string &str)
{
...
{
    assert(!line.empty());
    Position::StringVector values = Position::PUtils::SplitString(line, SPLITSTR);
    if (!values.empty())
    {
        Position::TrackerItem item;
...
void ImgAutoPrjList::save(const std::string &path)
{
    if (!path.empty() && open(path, ios::out))
    {
        ...
        {
            char gpsstr[50] = {0};
            const Position::BLHCoordinate &blh = mTrackerInfos[i].blh;
            if (Position::BLHCoordinate::isValid(blh))
            {
