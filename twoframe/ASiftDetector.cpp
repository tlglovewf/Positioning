...
void ASiftDetector::detectAndCompute(const Mat& img, std::vector< KeyPoint >& keypoints, Mat& descriptors)
{
  ...
  for(int tl = 1; tl < 4; tl++)
  {
    ...
    //   extractor.compute(timg, kps, desc);
      ...
      // Ptr<cv::xfeatures2d::SIFT> featurer = cv::xfeatures2d::SIFT::create(250,4,0.05,15,1.4);
      Ptr<cv::Feature2D> featurer = cv::xfeatures2d::SIFT::create(250,4,0.05,15,1.4);
      // featurer->detect(timg,kps,mask);
      // featurer->compute(timg,kps,desc);
      featurer->detectAndCompute(timg,mask,kps,desc);
