...
struct SIGNIMAGE//tracker_txt内容，包括有标牌的后两张图和trk_txt中
{
	int sum;
	int last1;
	int rec1;
...
bool ReadTrackerTxt(const string &trtxtfile, vector<SIGNIMAGE> &trtxts)
{
	vector<string> strs;
	...
	for (auto s : strs)
	{
		...
		//记录第14位及14位后两位数据，为了寻找对应图片和矩形框
		mtrt.sum = atoi(ss[13].c_str());
		mtrt.last1 = atoi(ss[14].substr(pos1 + 1, pos2).c_str());
...
int main()//测试代码
{
	...
	ReadTrackerTxt(trtxtPath, trtxts);
	...
	for (size_t i = 0; i < trtxts.size(); i++)//遍历tracker_txt
	{
		string trkfilePath1 = trkresultfolder + "/" + seqtxts[trtxts[i].last1] + ".trk";
		string trkfilePath2 = trkresultfolder + "/" + seqtxts[trtxts[i].last2] + ".trk";
